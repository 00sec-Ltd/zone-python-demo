...
class Zone {
  ...
  setListTable() {
    ...
    $("#table").prepend(
      compiled({ users: this.tableList.filter((num) => num.checked) })
    );
    this.setAggs();
  ...
  htmlAbnormal(value, show) {
    ...
    const abnormal = _.template(
      '<div class="abnormal"><h4>无法链接0.zone数据服务器</h4><h5>可能有以下原因</h5><h5>1.你的网络环境无法访问公开网络；</h5><h5>2.你的插件部署环境异常</h5></div>'
    );
    show
      ? $("#tableContent").html(compiled({ value }))
      : $(".zoen").html(abnormal());
  }
  ...
  setAggs() {
    ...
    $("#table").on("click", "th", function (event) {
      ...
      if ($(this).attr("type")) {
        ...
        if (aggs.length !== 0) {
          $("#aggsinput").html(
            compiled({ users: aggs, left: event.pageX, top: event.pageY })
          );
          $("#aggsList input[type='checkbox']").on("click", (e) => {
            if (e.target.checked) {
              ...
              _this.title = _this.title.replace(
                `&&(${$(this).attr("type")}=${e.target.name})`,
                ""
              );
              $("#screen_input").val(_this.title);
              _this.page = 1;
